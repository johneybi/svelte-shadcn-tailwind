...
<div class="flex min-h-screen flex-col items-center" style={`background: ${bg()};`}>
  <div class="flex w-full container flex-1 flex-col ">

        <Header />


    <div class="w-full mx-auto flex flex-1 bg-transparent">
      <aside class="hidden w-64 border-r bg-white md:block">
        <Sidebar />
      </aside>

      <main class="flex-1 overflow-y-auto bg-dusty-100 p-4 md:p-6 rounded-t-xl">
        {@render children()}
		</main>
    </div>
  </div>
